...
export default function MissionsWiki() {
    ...
    const getScaledValue = (base: number) => {
        ...
        return Math.floor(base * (1 + (clanPoints - 1) * (multiplier - 1)));
    };
            ...
            <div className="space-y-6 pt-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {filteredMissions.map((battle) => {
                        ...
                        const baseUnitPower = ((battle.BaseDamage * 8) + battle.BaseHealth) * 3;
                        const baseTeamPower = baseUnitPower * 3;
                        const suggestedPower = baseTeamPower * Math.pow(multiplier, clanPoints - 1);
                        ...
                        return (
                            <Card key={battle.MissionId} className={cn(
                                ...
                                <div className="p-5 border-b border-border/50 flex justify-between items-start bg-gradient-to-r from-bg-secondary/40 to-transparent">
                                    ...
                                    <div className={cn(
                                        "w-12 h-12 rounded-2xl flex items-center justify-center border-2 transition-all duration-500 group-hover:rotate-[360deg]",
                                        "bg-accent-primary/5 border-accent-primary/30"
                                    )}>
                                        <Sword className={cn("w-6 h-6", "text-accent-primary")} />
                                    </div>
                                </div>
